...
- As it mentions in the chapter, you'll first need to re-estimate your Step 4 model using the `ivreg` function from **AER**. Note that `ivreg` formula syntax is `y ~ d + x | z + x`, where `d` is treatment, `z` is the instrument, and `x` are controls. Controls must go on both sides of the `|`, unlike in `feols()`.
